...
for i1=1:size(lw,2)
    ...
    n=numel(x);
    xs=sort(x);
    xcutoff=xs(end-ceil(min(0.2*n,3*sqrt(n/Reff))));                        %Value of x where maximally 20% of samples (but mostly fewer) have higher likelihood (use more samples if Reff is lower)
    if xcutoff<log(realmin)
        % need to stay above realmin
        xcutoff=-700;
    end
    x1=x(x<=xcutoff);                                                       %Samples with low likelihood for this trial
    x2=x(x>xcutoff);                                                        %Samples with high likelihood for this trial
    n2=numel(x2);
    if n2<=4
    ...
    if k<1/3 || isinf(k) || isnan(k)                                        %DM: I added "isnan(k)" as condition, because this occurs occasionally (when Pareto fit fails)
