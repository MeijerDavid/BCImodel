function vbmcStructure = vbmc_wrapper(logPfun,x0,LB,UB,PLB,PUB,options,nAttempts,varargin)
...
init = sampleXnormal(nAttempts(2),meanX,jitterPLB,jitterPUB);                       %Sample initializations from multivariate normal around x0
...
while ~VBMCconvergedBool && (nFinished < nAttempts(2))
    ...
    %Call Variational Bayesian Monte Carlo
    [vp_all{nFinished,1},elbo_all(nFinished,1),elbo_sd_all(nFinished,1),exitflag_all(nFinished,1),output_all{nFinished,1}] = vbmc(logPfunwrapper,init(nFinished,:),LB,UB,vbmcPLB,vbmcPUB,options);
